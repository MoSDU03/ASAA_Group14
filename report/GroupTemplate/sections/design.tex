...
\begin{itemize}
    ...
    \item \textit{WaitingPosition} -> \textit{Filling} [positionValid AND cycleTime ≤ 200ms] / openValve()
    \item \textit{Filling} -> \textit{ClosingValve} [level ≥ 325ml AND fillTime ≤ 3000ms] / closeValve()
...
Timing constraints appear as state invariants (\textit{WaitingPosition}: cycleTime ≤ 200ms, \textit{Filling}: fillTime ≤ 3000ms) and transition guards, enabling direct mapping to UPPAAL clock constraints.
