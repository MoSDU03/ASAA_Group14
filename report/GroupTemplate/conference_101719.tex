...
We adopt a model-driven methodology based on EAST-ADL~\cite{eastadl_blom}. Our approach consists of five phases: requirements elicitation following quality attribute scenario templates~\cite{bass_swa}, architecture design using SysML notation, formal modeling using UPPAAL timed automata, verification using model checking, and implementation with empirical validation.
...
\begin{itemize}
    ...
    \item Environment: Normal operation (20C, nominal flow)
    \item Artifact: Fill controller
    \item Response: Complete detection to fill to release cycle
    \item Measure: 600ms <= cycle time <= 1500ms
\end{itemize}
...
Following EAST-ADL methodology, we begin with a feature model defining system variability. The root feature CanFillingSystem has four mandatory features: DetectCanPosition, ControlLiquidFlow, MonitorFillLevel, and LogOperations. Sensor type selection uses an XOR constraint between UltrasonicSensor and CapacitiveSensor. We selected ultrasonic for better reliability across liquid types. Two optional features, FaultDetection and EmergencyShutdown, are included with requires dependency between them.

\subsection{Analysis Architecture}

The analysis-level architecture uses SysML internal block diagram notation. Three analysis functions comprise the logical architecture:
...
The FillController state machine has six states: Idle, WaitingPosition, Filling, ClosingValve, Complete, and Fault.
...
\begin{itemize}
    ...
    \item Filling to ClosingValve [level >= 325ml AND fillTime < 3000ms] / closeValve()
...
Following Bass et al.'s architectural tactics framework~\cite{bass_swa}, we applied several specific techniques to meet our quality attributes. For performance, asynchronous messaging prevents any component from blocking others. We poll sensors at 20Hz, which balances quick response times with reasonable CPU usage. For safety, timeout watchdogs catch states that get stuck, and the emergency shutdown path bypasses normal control to close valves immediately. Reliability comes from detecting faults at multiple system levels and logging all events to PostgreSQL for analysis. Maintainability benefits from loose coupling through the message bus—each component has clear interfaces defined by topic schemas. Finally, testability improves dramatically with event logs providing complete trace data, and the sensor simulator lets us run controlled tests without physical hardware.

\section{Formal Verification}

\subsection{UPPAAL Model}

We translated the SysML state machine to a network of UPPAAL timed automata. The FillController template contains six locations matching the states, with two clocks: fill\_clock measuring filling duration and cycle\_clock measuring total cycle time.

Location invariants enforce timing bounds: Filling has invariant fill\_clock <= 3000 and WaitingPosition has cycle\_clock <= 200. These prevent the model from remaining in time-bounded states beyond specified limits.
...
Table~\ref{tab:verification} lists 10 verified properties linking to requirements. We use UPPAAL's CTL query language where A[] means "for all paths always," E<> means "there exists a path where eventually," and A<> means "for all paths eventually."
...
Q3 & A[] Filling imply fill clock <= 3000 & Satisfied \\
Q4 & A[] WaitPos imply cycle clock <= 200 & Satisfied \\
...
\textit{Fix:} Added guard fill\_clock <= 3000 to transition. After correction, Q3 verified successfully.
...
\textit{Fix:} Added location invariant cycle\_clock <= 200. This forces a transition (either to Filling if valid, or to Fault if timeout). After correction, Q4 verified successfully.
...
We implemented the architecture using Docker with four services: PostgreSQL database (persistent storage), Eclipse Mosquitto MQTT broker (QoS 1), sensor simulator (Python, generates realistic sensor data), and fill controller (Python, implements state machine).
...
We tested under controlled conditions: 20C ambient temperature, standard flow rates, and no physical wear on components. Production deployment would encounter temperature variation, viscosity changes, and component degradation over time. Long-term reliability needs validation beyond our 112 test cycles.
...
\bibitem{bass_swa}
L.~Bass, P.~Clements, and R.~Kazman, \emph{Software Architecture in Practice}, 3rd~ed.\hskip 1em plus 0.5em minus 0.4em\relax Addison-Wesley, 2012.

\bibitem{friedenthal2014}
S.~Friedenthal, A.~Moore, and R.~Steiner, \emph{A Practical Guide to SysML: The Systems Modeling Language}, 3rd~ed.\hskip 1em plus 0.5em minus 0.4em\relax Morgan Kaufmann, 2014.

\bibitem{jepsen2019}
S.~C. Jepsen et al., ``Industry 4.0 middleware software architecture interoperability analysis,'' 2019.

\bibitem{buschmann2007}
F.~Buschmann, K.~Henney, and D.~C. Schmidt, ``Past, present, and future trends in software patterns,'' \emph{IEEE Software}, 2007.

\bibitem{bengtsson2004}
